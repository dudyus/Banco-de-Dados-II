...
FROM aluno a 
JOIN matricula m ON a.id = aluno_id
...
FROM curso c 
LEFT JOIN matricula m ON c.id = curso_id --left join pra aparecer TODAS as materias msm as que n tem alunos, caso fosse INNER JOIN as que tem 0 alunos n apareceriam
GROUP BY c.nome;
